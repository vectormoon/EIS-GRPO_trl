...
class GRPOTrainer(Trainer):
    ...
    # TODO: equivalent transformations for prompt
    def transform_prompt(self, prompt, equivalent_state_idx):
        if equivalent_state_idx == 0:
            return prompt
        elif equivalent_state_idx == 1:
            return prompt.replace(
                "Do you know the answer to this question? If you know, please answer \"Yes, I know.\", if you don't know, please answer \"No, I don\'t know.\".\n",
                "Do you know the answer to this question? If you don't know, please answer \"No, I don\'t know.\", if you know, please answer \"Yes, I know.\".\n"
            )
        elif equivalent_state_idx == 2:
            return prompt.replace(
                "Do you know the answer to this question? If you know, please answer \"Yes, I know.\", if you don't know, please answer \"No, I don\'t know.\".\n",
                "Based on your knowledge, do you know the answer to the following question? If you know, please answer \"Yes, I know.\", if you don't know, please answer \"No, I don\'t know.\".\n"
            )
        elif equivalent_state_idx == 3:
            return prompt.replace(
                "Do you know the answer to this question? If you know, please answer \"Yes, I know.\", if you don't know, please answer \"No, I don\'t know.\".\n",
                "Based on your knowledge, do you know the answer to the following question? If you don't know, please answer \"No, I don\'t know.\", if you know, please answer \"Yes, I know.\".\n"
            )
    ...
    def _generate_and_score_completions(
        ...
        # TODO: Generate equivalent state for every prompt
        transformed_prompts = []
        
        # [1,1,1,1]->[1,2,1,2], 2 is equivalent template
        # for prompt_idx in range(0, len(prompts), self.num_generations):
        #     for equivalent_idx in range(self.num_generations):
        #         transformed_prompts.append({'prompt': self.transform_prompt(prompts[prompt_idx], int(equivalent_idx%self.args.num_equivalent_states))})

        # [1,1,1,1]->[1,1,2,2], 2 is equivalent template
        for prompt_idx in range(0, len(prompts), self.num_generations):
            repeats_per_state = self.num_generations // self.args.num_equivalent_states
            for state_idx in range(self.args.num_equivalent_states):
                for _ in range(repeats_per_state):
                    transformed_prompts.append({'prompt': self.transform_prompt(prompts[prompt_idx], state_idx)})
        prompts_text = [
            maybe_apply_chat_template(example, self.processing_class)["prompt"]
            for example in transformed_prompts
        ]
        ...
        if self.use_vllm:
            ...
            # TODO: Get token probabilities
            scores = torch.stack(outputs.scores, dim=1)  # (batch_size, seq_len, vocab_size)
            probs = torch.softmax(scores, dim=-1)  # 转换为概率
    
            # Get token probabilities for the completion tokens
            token_probs = torch.gather(probs, dim=2, index=outputs.sequences[:,prompt_ids.size(1):].unsqueeze(-1)).squeeze(-1)      
            ...
            with profiling_context(self, reward_func_name):
                if isinstance(
                    ...
                    if is_conversational(inputs[0]):
                        ...
                        # TODO: transformed_prompt->prompts
                        # texts = [p + c for p, c in zip(prompts, completions)]
                        texts = [p['prompt'] + c for p, c in zip(transformed_prompts, completions)]
                    ...
                    # TODO: add token_probs to reward_kwargs
                    reward_kwargs["token_probs"] = token_probs
                    
                    # TODO: transformed_prompt->prompts
                    output_reward_func = reward_func(
                        # prompts=prompts, 
                        prompts=[p['prompt'] for p in transformed_prompts],
        ...
        # TODO: compute grouped rewards
        # Classified by equivalence state and prompt
        rewards_reshaped = rewards.view(-1, self.num_generations//self.args.num_equivalent_states, self.args.num_equivalent_states)
        
        # compute global advantages
        global_mean = rewards_reshaped.mean(dim=(1,2)).unsqueeze(-1).unsqueeze(-1)
        global_std = rewards_reshaped.std(dim=(1,2)).unsqueeze(-1).unsqueeze(-1)
        global_advantages = (rewards_reshaped - global_mean) / (global_std + 1e-4)
        
        # compute local advantages (every equivalent state)
        local_mean = rewards_reshaped.mean(dim=2, keepdim=True)
        local_std = rewards_reshaped.std(dim=2, keepdim=True)
        local_advantages = (rewards_reshaped - local_mean) / (local_std + 1e-4)
        
        advantages = (global_advantages + local_advantages).reshape(-1)
        # END
        ...
        # TODO: # Add new reward calculation based on rewards_reshaped
        # Calculate mean and std for each equivalent state group
        for i in range(self.args.num_equivalent_states):
            state_rewards = rewards_reshaped[:, :, i]
            self._metrics[mode][f"reward/equivalent_state_{i}/mean"].append(state_rewards.mean().item())
            self._metrics[mode][f"reward/equivalent_state_{i}/std"].append(state_rewards.std().item())

        # Calculate overall statistics
        self._metrics[mode]["reward/global_mean"].append(rewards_reshaped.mean().item())
        self._metrics[mode]["reward/global_std"].append(rewards_reshaped.std().item())

        # Add advantage statistics
        self._metrics[mode]["advantage/global_mean"].append(global_advantages.mean().item()) 
        self._metrics[mode]["advantage/global_std"].append(global_advantages.std().item())
        self._metrics[mode]["advantage/local_mean"].append(local_advantages.mean().item())
        self._metrics[mode]["advantage/local_std"].append(local_advantages.std().item())
        # END
